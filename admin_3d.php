...
if($_SERVER['REQUEST_METHOD']=='POST'){
    if (isset($_POST['snoEdit'])){ // Check if edit form is submitted
        ...
        $d3D_Technology = $_POST["d3D_TechnologyEdit"];
        $Material = $_POST["MaterialEdit"];
        $Color = $_POST["ColorEdit"];
        $Surface_Finish = $_POST["Surface_FinishEdit"];
        $Qty = $_POST["qtyEdit"];
        ...
        //  SQL update query
        $sql = "UPDATE `3dprint` SET `d3D_Technology` = '$d3D_Technology', `Material` = '$Material',`Color` = '$Color', `Surface_Finish` = '$Surface_Finish', `Qty` = '$Qty', `Qty` = '$Qty', `Product_Use` = '$Product_Use', `Build_Time` = '$Build_Time', `Package_Desc` = '$Package_Desc'WHERE `3dprint`.`sno` = $sno";
...
<body>
  <!-- Modal for editing records -->
            ...
            <form action="/printzer/admin_welcome.php" method="POST"> <!-- Form for editing a record -->
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="d3D_Technology">d3D_Technology</label>
                        <textarea class="form-control" id="d3D_TechnologyEdit" name="d3D_TechnologyEdit" rows="1"></textarea>
                    </div>
                    <div class="form-group">
                        <label for="Material">Material</label>
                        <input type="text" class="form-control" id="MaterialEdit" name="MaterialEdit" aria-describedby="emailHelp">
                    </div>
                    <div class="form-group">
                        <label for="Color">Color</label>
                        <textarea class="form-control" id="ColorEdit" name="ColorEdit" rows="1"></textarea>
                    </div>
                    <div class="form-group">
                        <label for="Surface_Finish">Surface_Finish</label>
                        <textarea class="form-control" id="Surface_FinishEdit" name="Surface_FinishEdit" rows="1"></textarea>
                    </div>
                    <div class="form-group">
    ...
    <ul class="nav nav-pills flex-column">
        ...
        <li class="nav-item">
            <a class="nav-link active" href="admin_3d.php" style="width:150px;">3D Printing</a>
        </li>
        <li class="nav-item">   
    ...
    <div class="d2">
        <h3>Quotation required By Users for 3D Printing</h3>
        ...
        <thead>
            ...
            <th scope="col">d3D_Technology</th>
            <th scope="col">Material</th>
            <th scope="col">Color</th>
            <th scope="col">Surface_Finish</th>
            <th scope="col">qty</th>
        ...
        <tbody>
            <?php
            $sql="select * from 3dprint"; // Query to fetch all records from the 'information' table
            ...
            while($row=mysqli_fetch_assoc($result)){ // Loop through each record
                ...
                echo"<tr>
                    ...
                    <td>". $row['username'] . "</td>
                    <td>". $row['filename'] . "</td>
                    <td>". $row['d3D_Technology'] . "</td>
                    <td>". $row['Material'] . "</td>
                    <td>". $row['Color'] . "</td>
                    <td>". $row['Surface_Finish'] . "</td>
                    <td>". $row['Qty'] . "</td>
...
Array.from(edits).forEach((element) => {
    element.addEventListener("click", (e) => {
        ...
        d3D_Technology = tr.getElementsByTagName("td")[3].innerText; // Updated index to match table column
        Material = tr.getElementsByTagName("td")[4].innerText;
        Color = tr.getElementsByTagName("td")[5].innerText;
        Surface_Finish = tr.getElementsByTagName("td")[6].innerText;
        ...
        document.getElementById('d3D_TechnologyEdit').value = d3D_Technology;
        document.getElementById('MaterialEdit').value = Material;
        document.getElementById('ColorEdit').value = Color;
        document.getElementById('Surface_FinishEdit').value = Surface_Finish;
